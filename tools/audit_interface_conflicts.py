...
def audit(root=ROOT):
    ...
    resolutions = [
        ...
        {'id':'GLOBAL_TYPE:resource_ptr_table','outcome':'UNIFIED','resolution':'char far *resource_ptr_table[] spelled out consistently: src/LEVEL.C\'s hoisted top-of-file declaration (line 29, previously plain `char *`) now says `far` explicitly, matching its own F_B99F-block declaration and src/BOARD.C/src/GAME.C\'s `resource_ptr_table_build()`; the redundant second LEVEL.C declaration (formerly at line ~220, marked as an "alternate view") is removed so the merged unit carries one declaration for the symbol. src/MENURES.C\'s declarator (previously bare `*resource_ptr_table[]` trailing a `far *` list) now repeats `far` explicitly too. Under -mc an unqualified data pointer already defaults to far, so this was spelling only. C_AF45_C15E probe: EXACT (4699/4699 bytes). F_1D47 probe: EXACT (350/350 bytes).'},
        {'id':'RECORD:R','outcome':'UNIFIED','resolution':'struct tag `R` was reused across five unrelated file-scoped structs with different layouts (src/HELPMENU.C, src/LEVEL.C, src/MENULIST.C, src/PUZZLE.C, src/SCORE.C). Tags are file-scoped and are not emitted into the object file, so renaming is byte-neutral; each was given a distinct descriptive name instead of leaving five identically-named but incompatible tags: src/HELPMENU.C -> `struct dlg_pick_desc` (dialog list-pick descriptor: pointer/array/count), src/LEVEL.C -> `struct actor_rec` (23-byte actor record walked by chapter_map_sprites_wipe), src/MENULIST.C -> `struct catalog_entry` (the local record-catalog entry F_7BFC casts gc0fe through; include/GC0FE.H and include/GC316.H comments updated to the new name), src/PUZZLE.C -> `struct piece_desc` (2-byte a/b piece descriptor; include/GC316.H comment updated), src/SCORE.C -> `struct score_pos` (x/y score-panel position pair). No field layouts changed. Probes: C_D3DA_D49D (HELPMENU.C) EXACT (217/217 bytes), C_AF45_C15E (LEVEL.C) EXACT (4699/4699 bytes), F_7BFC (MENULIST.C) EXACT (405/405 bytes), C_8A37_969D (PUZZLE.C) EXACT (3605/3605 bytes), C_9962_99E2 (SCORE.C) EXACT (172/172 bytes).'},
    ]
    findings = []
