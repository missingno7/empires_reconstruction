...
Full EXE identity is preserved. Follow-ups to MVP1 remove 6,671 bytes from raw
fallback: 4,623 bytes of matching C/library regions, the 512-byte header,
...
The combined EXE metrics distinguish 156 source proof units from **zero proven
historical source modules**, and 814 declared owner-symbol bindings from
**zero linker-resolved bindings**. Hash-pinned on-disk machine extents classify
34,674 raw EXE bytes as unresolved machine code; overlapping entry/parent
extents count once. The other 21,276 raw bytes remain unknown. Two palette owners account for
1,536 embedded-asset bytes. There are now 201 component-owned bindings
within fixed placement: two palette references, 151 C/ASM entry references,
and 38 library public references from earlier checkpoints, plus ten owned references
in the new C wave. See [embedded-palettes.md](embedded-palettes.md).

[linkage-blockers.json](linkage-blockers.json) snapshots 26 held upstream
candidates (10,345 extent bytes): 24 linkage refusals and two byte differences,
...
Sixty-three tests pass, including independent hand-authored codec streams,
...
DAT payloads and 55,950 raw EXE bytes remain temporary fallbacks. See
