"""Fresh complete-extent proof for recovered F_CDDD."""
...
class MatchingCWave51Tests(unittest.TestCase):
    def test_recovered_tail_and_fixups(self):
        manifest = read_json(ROOT / 'layout/manifest.json')
        recipe = read_json(ROOT / 'recipes/c/matching-wave51.json')
        owner = next(r for r in manifest['regions'] if r['id'] == 'F_CDDD')
        self.assertEqual(next(r for r in recipe['owners'] if r['id'] == 'F_CDDD'), owner)
        ...
        with tempfile.TemporaryDirectory(dir=ROOT / 'build') as temporary:
            ...
            self.assertEqual(data[-5:], bytes.fromhex('33c0eb00c3'))
