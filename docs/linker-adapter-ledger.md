...
| Turbo C-compatible empty DGROUP metadata | Standalone TASM owners do not emit Turbo C's empty `_DATA`/`_BSS` declarations and `DGROUP` membership | Keeps each freshly assembled code contribution in the same linker group topology as the surrounding Turbo C objects | Make the recovered source/module itself emit the historical group declarations | **Active and narrowly checked**; adds only zero-length `_DATA`/`_BSS` SEGDEFs and `DGROUP` membership, while rejecting any change to TEXT bytes, publics, externals or fixups |
