...
extern int value_parity(), resource_load_record();extern void f03cc();extern void f03c9();extern void f03b4();
extern void f2986();
...
void board_redraw_paint()
{
    ...
    if (value_parity(g9ade) != 0) {
        ...
        f03cc(8, 0xc8, ui_gfx_shadow_a, 0);
    }
    n = *(q = f2a70());
    p = q + 1;
    for (i = 0; i < n; i++, p += 3)
    ...
    if (board_records[0x3e7] == board_record_index + 1) {
        f03cc((w8 = board_records[0x3e5]) * 2, (j = board_records[0x3e6]) + 0xb8, s735e, 0);
        fd825(7, w8, j, 8, 0x10);
    }
