"""Fresh complete-extent proof for the recovered F_CE2A C routine."""
...
class MatchingCWave50Tests(unittest.TestCase):
    def test_recovered_complete_extent_and_fixups(self):
        manifest = read_json(ROOT / 'layout/manifest.json')
        recipe = read_json(ROOT / 'recipes/c/matching-wave50.json')
        owner = next(r for r in manifest['regions'] if r['id'] == 'F_CE2A')
        recipe_owner = next(r for r in recipe['owners'] if r['id'] == 'F_CE2A')
        ...
        with tempfile.TemporaryDirectory(dir=ROOT / 'build') as temporary:
            ...
            self.assertEqual(len(proof['fixups']), 11)
            self.assertEqual(proof['load_relocations'], [])
            self.assertEqual(data[-6:], bytes.fromhex('33c0eb005ec3'))
