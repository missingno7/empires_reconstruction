...
class MatchingCWave78Tests(unittest.TestCase):
    def test_complete_extent_and_drive_fixup(self):
        manifest = read_json(ROOT / 'layout/manifest.json')
        recipe = read_json(ROOT / 'recipes/c/matching-wave78.json')
        owner = next(r for r in manifest['regions'] if r['id'] == 'F_652A')
        self.assertEqual(next(r for r in recipe['owners'] if r['id'] == 'F_652A'), owner)
        ...
        with tempfile.TemporaryDirectory(dir=ROOT / 'build') as temporary:
            ...
            self.assertEqual(proof['fixups'][0]['target'], '_gc0c8')
