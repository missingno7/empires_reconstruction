...
| Anchored TASM BSS reserve | Historical BSS translation units remain mostly unknown | Supplies the verified 37,250-byte reserve and 247 canonical public anchors through real TASM OMF | Partition storage and publics among evidenced historical modules | **Reduced**: twelve symbolic contributions now own 3,524 bytes: `F01CEBSS.OBJ`, `ROWPTRS.OBJ`, `RSTATEB.OBJ`, `G40D4B.OBJ` for the 672-byte resource-25 workspace loaded by `F_1D47`, `SLOTBSS.OBJ`, `ANIMBSS.OBJ`, `GC360BSS.OBJ`, `C470BSS.OBJ`, the four-flag state, paired 96-byte note tables, and `VOICEPTR.OBJ` for the eleven-entry far-pointer table initialized by `F_DEFA`. Seven `GAMEBSS` fragments hold the remaining 33,726-byte anchored remainder. The canonical map and contribution plan are both checked against linker bindings. |
