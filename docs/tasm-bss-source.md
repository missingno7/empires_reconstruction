...
ten recovered 27-byte records at bytes 35,694–35,963. Two `GAMEBSS` fragments
hold the remaining 34,978-byte anchored reserve. The plan must cover the full logical reserve, and
...
evidence on every structural link, but 34,978 bytes remain aggregate storage.
