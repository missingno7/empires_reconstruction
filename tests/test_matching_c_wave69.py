"""Fresh complete-extent proof for the recovered F_019C DOS-write C routine."""
...
class MatchingCWave69Tests(unittest.TestCase):
    def test_recovered_complete_subextent(self):
        ...
        recipe = read_json(ROOT / 'recipes/c/matching-wave69.json')
        owner = next(r for r in manifest['regions'] if r['id'] == 'F_019C')
        self.assertEqual(next(r for r in recipe['owners'] if r['id'] == 'F_019C'), owner)
        ...
        with tempfile.TemporaryDirectory(dir=ROOT / 'build') as temporary:
            ...
            self.assertEqual(data, bytes.fromhex('b4 40 bb 02 00 cd 21 c3'))
