    ...
    if expose_internal_labels:
        ...
        for owner_id, receipt in receipts.items():
            ...
            for external in module.externals:
                declared = owner_by_id[owner_id].get('build', {}).get('bindings', {}).get(external, {})
                if declared.get('owner'):
                    # Component-owned bindings already identify their target;
                    # do not reinterpret a numeric-looking public as an address.
                    continue
        ...
        # Older manifests named this owner numerically. Current source exports
        # the conventional Turbo C entry directly, so no adapter is needed.
        if 'F_4A93' in owner_by_id and '_main' not in explicit_publics:
    ...
    for region in sorted(manifest['regions'], key=lambda item: item['start']):
        if region['id'] in owner_by_id:
            ...
            scaffold.append({
                ...
                **({'internal_labels': internal_labels[owner['id']]}
                   if expose_internal_labels and owner['id'] in internal_labels else {}),
