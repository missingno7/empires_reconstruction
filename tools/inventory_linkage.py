...
def capture(upstream, root=ROOT):
    ...
    owned = {r['id'] for r in read_json(root / 'layout/manifest.json')['regions'] if r['kind'] != 'RAW'}
    held = [r for r in evidence['held'] if r['id'] not in owned]
    symbols = {}
    for entry in held:
    ...
    report = {'format': 'empires-held-linkage-evidence-v1', 'upstream': str(upstream),
              ...
              'scope': 'Historical provider refusals, excluding currently owned IDs; fresh compilation still required.',
              ...
              'symbols': ranking, 'candidates': held}
