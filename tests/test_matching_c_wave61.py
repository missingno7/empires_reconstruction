"""Fresh complete-extent proof for the recovered F_C232 C/inline-assembler routine."""
...
class MatchingCWave61Tests(unittest.TestCase):
    def test_recovered_complete_extent_and_fixups(self):
        manifest = read_json(ROOT / 'layout/manifest.json')
        recipe = read_json(ROOT / 'recipes/c/matching-wave61.json')
        owner = next(r for r in manifest['regions'] if r['id'] == 'F_C232')
        self.assertEqual(next(r for r in recipe['owners'] if r['id'] == 'F_C232'), owner)
        ...
        with tempfile.TemporaryDirectory(dir=ROOT / 'build') as temporary:
            ...
            self.assertEqual(data[:6], bytes.fromhex('55 8b ec 33 c0 bb'))
            self.assertEqual(data[-9:], bytes.fromhex('bc 17 83 c6 02 e2 cf 5d c3'))
