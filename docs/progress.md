...
animation-state block, GC360 ten-record table, four-flag state array, paired 96-byte note tables, and the 44-byte eleven-voice pointer table initialized by `F_DEFA` are compatible symbolic TASM contributions; the 34,398-byte remainder retains the canonical anchor map.
...
|---|---:|---:|---:|
| Freshly compiled matching C | 14,077 | 57,936 | 323 |
| Freshly assembled matching ASM | 2,456 | 1,043 | 24 |
...
## One-hundred-seventeenth matching-C conversion

F_1ECD converts from its verified ASM owner to matching C. The complete
74-byte far rectangle-queue drain preserves six fixups and no loader
relocations. See [the wave 117 proof](matching-c-wave117.md).

Current matching-C coverage is 56,935 bytes across 331 owners, with 1,960
matching-ASM bytes across 14 owners. The raw frontier remains 10,173 bytes.

## One-hundred-eighteenth matching-C conversion

F_C1F7 converts from its verified ASM owner to matching C. The complete
59-byte sound voice pump preserves thirteen fixups and no loader relocations.
...
F_C27D converts from its verified ASM owner to matching C. The complete
109-byte voice service loop preserves eighteen fixups and no loader
relocations. See [the wave 119 proof](matching-c-wave119.md).

Current matching-C coverage is 57,103 bytes across 333 owners, with 1,792
matching-ASM bytes across 12 owners. The raw frontier remains 10,173 bytes.

## One-hundred-twentieth matching-C conversion

F_CAF1 converts from its verified ASM owner to matching C. The complete
87-byte sound tick service body preserves fourteen fixups and no loader
relocations. See [the wave 120 proof](matching-c-wave120.md).
...
## One-hundred-twenty-second matching-C conversion

F_D85F converts from its verified ASM owner to matching C. The complete
59-byte record deletion preserves two fixups and no loader relocations. See
[the wave 122 proof](matching-c-wave122.md).

Current matching-C coverage is 57,316 bytes across 336 owners, with 1,579
matching-ASM bytes across 9 owners. The raw frontier remains 10,173 bytes.

## One-hundred-twenty-third matching-C conversion

F_D89A converts from its verified ASM owner to matching C. The complete
86-byte rectangle hit-test preserves one fixup and no loader relocations. See
[the wave 123 proof](matching-c-wave123.md).
...
## One-hundred-twenty-fifth matching-C conversion

F_6D86 converts from its verified ASM owner to matching C. The complete
63-byte far RLE stage preserves its string-operation frame without fixups or
loader relocations. See [the wave 125 proof](matching-c-wave125.md).

Current matching-C coverage is 57,552 bytes across 339 owners, with 1,343
matching-ASM bytes across 6 owners. The raw frontier remains 10,173 bytes.

## One-hundred-twenty-sixth matching-C conversion

F_C914 converts from its verified ASM owner to matching C. The complete
116-byte music-stream decoder preserves sixteen fixups and no loader
relocations. See [the wave 126 proof](matching-c-wave126.md).
...
The raw frontier is now 8,651 bytes across 15 owners.

## One-hundred-thirty-fourth executable-data wave

Wave 134 partitions 99 bytes from isolated alignment bytes, a zero-initialized
run and four terminated ASCII records. The full EXE and both DAT archives remain
byte-identical.

The raw frontier is now 8,552 bytes across 15 owners.
...
The raw frontier is now 5,397 bytes across 16 owners.

## One-hundred-thirty-sixth executable-data wave

Wave 136 partitions a 28-entry, 56-byte little-endian pointer table from the
numeric data span. Its strictly increasing code targets and relocation-free
extent make the boundary independently verifiable; the following mixed table
remains raw. The full EXE and both DAT archives remain byte-identical.

The raw frontier is now 5,341 bytes across 17 owners.

## One-hundred-thirty-seventh executable-data wave

Wave 137 partitions the 28 one-byte attributes paired with wave 136's pointer
targets. The fixed-record boundary ends before the next mixed record stream;
the full EXE and both DAT archives remain byte-identical.

The raw frontier is now 5,313 bytes across 18 owners.

## One-hundred-thirty-eighth executable-data wave

Wave 138 partitions 46 fixed four-byte records and a terminal three-byte zero
pad from the numeric area. Their exact boundaries preserve the next raw owner;
the full EXE and both DAT archives remain byte-identical.

The raw frontier is now 5,126 bytes across 16 owners.

## One-hundred-thirty-ninth executable-data wave

Wave 139 partitions two isolated zero-initialized boundary components from the
...
## One-hundred-fortieth executable-data wave

Wave 140 partitions two owner-start zero runs totaling 22 bytes. The full EXE
...
## One-hundred-forty-second executable-data wave

Wave 142 decodes the complete 51-byte player-name instruction at `RAW_011037`
