...
Active production ASM modules: **18**, **12336** bytes.

| Classification | Modules |
|---|---|
| PROBABLE_ORIGINAL_ASM | 17 |
...
| F_4EEB | asm/F_4EEB.ASM | F_4EEB | 120 | PROBABLE_ORIGINAL_ASM | HIGH | True |
...
- **F_4B0C** is `RECONSTRUCTION_ONLY_ASM`: production already replaced the raw `asm db` capsule with symbolic TASM (labels, branches, named calls); `recovery/src/F_4B0C.C` is now an inactive capsule-only reference (see capsules table).
...
- **M_6B1A_6B4A** moves to `RECONSTRUCTION_ONLY_ASM`: both members already have byte-exact inline-asm C reconstructions (`recovery/src/F_6B4A.C` in production; `F_6B1A`'s historical reconstruction recorded in `layout/manifest.json`), so what remains is packaging (merge into one C unit and promote), not origin research.

## Capsules (opaque instruction bytes)

| File | Lines | Bytes | Classification | Reason |
|---|---|---|---|---|
| recovery/src/DOS_STUB.C | 4-22 | 404 | LEGITIMATE_DATA | Literal MS-DOS EXE header stub (loader preamble), vendor/linker boilerplate that must be byte-exact by nature. Outside `production-plan.json` (lives in `manifest.json` as `startup-stub`); included for completeness. |
| recovery/src/F_4B0C.C | 4-15+ | 915 | INSTRUCTION_CAPSULE | Whole-function `asm db` capsule for the F_4B0C opcode interpreter. Already resolved in production: `asm/F_4B0C.ASM` replaced it with symbolic TASM; `recovery/src/F_4B0C.C` is now an inactive reference, excluded from production scope but recorded here as the capsule type the task asks to flag. |
...
4. **M_6B1A_6B4A merge-and-promote**: combine F_6B1A's historical inline-asm C reconstruction (`layout/manifest.json` id `F_6B1A`) with `recovery/src/F_6B4A.C` into one compiled unit matching the module's object layout.
