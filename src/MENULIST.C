...
struct catalog_entry{char *a;int b;char pad[12];int c;};struct S{int n;struct catalog_entry *p;};extern struct S *gc0fe;
void menu_list_draw(int n)
{int a,b,i,j;struct catalog_entry r;int w;register int x,y;
a=cur_color_index_get();b=sprite_sheet_index_get();sprite_sheet_select(0);gfx_color_select(0);gfx_clear_rect(0,0,320,13);gfx_color_select(15);gfx_clear_rect(1,2,318,10);
