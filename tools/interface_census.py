...
def argument_type(arg):
    ...
    if re.fullmatch(r'(?:unsigned|signed)?\s*(?:char|int|short|long|float|double)|(?:struct|union|enum)\s+\w+', arg): return norm(arg)
...
def object_declarations(statement):
    ...
    for member in tail.split(','):
        ...
        if not item: return []
        typ=norm(base+' '+item[1]); array=item[3].strip() or None
        result.append({'symbol':item[2],'type':typ,'array':array,
                       'pointer_shape':'near' if 'near' in typ else 'far' if '*' in typ else None,
                       'size':type_size(typ) if not array else None})
    return result
...
def type_size(typ):
    if '*' in typ: return 2 if 'near' in typ else 4
