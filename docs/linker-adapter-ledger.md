...
| Source-generated `STRLEN` library replacement | A matching-C proof temporarily replaced the selected `CC.LIB` module | Preserved the library extraction position during the original experiment | Direct unresolved `_strlen` demand should select pinned `CC.LIB` `STRLEN` | **Removed.** `LIB_STRLEN` is again a legitimate pinned `CC.LIB` owner; its matching-C source remains an independent regression proof. |
...
| Candidate DATA/code object interleaving | Historical object boundaries and response-file order were partly unknown | Reproduced the four observed cross-segment relocation-order constraints | Canonical compatible source-DATA module plan | **Removed.** [`canonical-link-plan.json`](../recipes/data/canonical-link-plan.json) supplies ordinary response-file order during source-DATA construction. No post-hoc object movement follows. Historical module identity remains open. |
| Arithmetic FIXUPP subrecord ordering | The former shared Turbo C arithmetic object emitted ascending explicit fixups | Reproduced the historical descending relocation run | Complete symbolic arithmetic module | **Removed.** [`M_DDD9_DF98`](../asm/M_DDD9_DF98.ASM) is a 700-byte compatible reconstructed TASM module. Its descending `ORG` contributions naturally emit all 17 historical FIXUPP records in order, including six DGROUP-frame data references. No OMF rewrite occurs. |
...
scaffold still differs; the direct source-DATA response-plan path is the exact result.
