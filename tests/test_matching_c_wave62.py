"""Fresh complete-extent proof for the recovered F_CA91 C/inline-assembler routine."""
...
class MatchingCWave62Tests(unittest.TestCase):
    def test_recovered_complete_extent_and_fixups(self):
        manifest = read_json(ROOT / 'layout/manifest.json')
        recipe = read_json(ROOT / 'recipes/c/matching-wave62.json')
        owner = next(r for r in manifest['regions'] if r['id'] == 'F_CA91')
        self.assertEqual(next(r for r in recipe['owners'] if r['id'] == 'F_CA91'), owner)
        ...
        with tempfile.TemporaryDirectory(dir=ROOT / 'build') as temporary:
            ...
            self.assertEqual(data, bytes.fromhex('55 8b ec 32 e4 a3 92 1e 5d c3'))
