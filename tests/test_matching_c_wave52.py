"""Fresh complete-extent proof for the recovered F_CE00 C routine."""
...
class MatchingCWave52Tests(unittest.TestCase):
    def test_recovered_complete_extent_and_fixups(self):
        manifest = read_json(ROOT / 'layout/manifest.json')
        recipe = read_json(ROOT / 'recipes/c/matching-wave52.json')
        owner = next(r for r in manifest['regions'] if r['id'] == 'F_CE00')
        recipe_owner = next(r for r in recipe['owners'] if r['id'] == 'F_CE00')
