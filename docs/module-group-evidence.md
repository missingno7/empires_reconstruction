...
`F_C755` now has a separately proved symbolic TASM representation. Its former
C-only ten-owner experiment was split at that entry. The prefix
`C_C5D1_C706` is now superseded in the canonical path by
`M_C5D1_C706`: a 388-byte symbolic TASM module containing the command decoder,
speaker-gate controller, voice-disable helper, and PIT-output helper. It has
public offsets 0/167/232/309 and no OMF fixups. The direct calls outside the
contribution remain source-level same-segment expressions because that is how
