"""Fresh complete-extent proof for the F_B593 far-pointer cleanup routine."""
...
class MatchingCWave45Tests(unittest.TestCase):
    def test_fresh_far_pointer_cleanup_extent_and_fixups(self):
        manifest = read_json(ROOT / 'layout/manifest.json')
        owner = next(r for r in manifest['regions'] if r['id'] == 'F_B593')
        ...
        with tempfile.TemporaryDirectory(dir=ROOT / 'build') as temporary:
            ...
            self.assertEqual(len(data), 124)
            self.assertEqual(len(proof['fixups']), 28)
