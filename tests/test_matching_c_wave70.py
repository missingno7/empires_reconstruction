"""Fresh complete-extent proof for the recovered F_C877 timer-slice C routine."""
...
class MatchingCWave70Tests(unittest.TestCase):
    def test_recovered_complete_extent_and_fixups(self):
        manifest = read_json(ROOT / 'layout/manifest.json')
        recipe = read_json(ROOT / 'recipes/c/matching-wave70.json')
        owner = next(r for r in manifest['regions'] if r['id'] == 'F_C877')
        self.assertEqual(next(r for r in recipe['owners'] if r['id'] == 'F_C877'), owner)
