...
| Anchored TASM BSS reserve | Historical BSS translation units remain mostly unknown | Supplies the verified 37,250-byte reserve and 247 canonical public anchors through real TASM OMF | Partition storage and publics among evidenced historical modules | **Reduced**: eight symbolic contributions now own 2,852 bytes: `F01CEBSS.OBJ`, `ROWPTRS.OBJ`, `RSTATEB.OBJ`, `SLOTBSS.OBJ`, `ANIMBSS.OBJ`, `GC360BSS.OBJ`, `C470BSS.OBJ`, the four-flag state, paired 96-byte note tables, and `VOICEPTR.OBJ` for the eleven-entry far-pointer table initialized by `F_DEFA`. Seven `GAMEBSS` fragments hold the remaining 34,398-byte anchored remainder. The canonical map and contribution plan are both checked against linker bindings. |
