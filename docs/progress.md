...
animation-state block, GC360 ten-record table, four-flag state array, paired 96-byte note tables, the 44-byte eleven-voice pointer table initialized by `F_DEFA`, and the 672-byte resource-25 workspace filled by `F_1D47` are compatible symbolic TASM contributions; the 33,726-byte remainder retains the canonical anchor map.
