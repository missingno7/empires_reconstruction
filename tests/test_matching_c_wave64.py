"""Fresh complete-extent proof for the recovered F_034F BIOS-mode C routine."""
...
class MatchingCWave64Tests(unittest.TestCase):
    def test_recovered_complete_extent(self):
        manifest = read_json(ROOT / 'layout/manifest.json')
        recipe = read_json(ROOT / 'recipes/c/matching-wave64.json')
        owner = next(r for r in manifest['regions'] if r['id'] == 'F_034F')
        self.assertEqual(next(r for r in recipe['owners'] if r['id'] == 'F_034F'), owner)
        ...
        with tempfile.TemporaryDirectory(dir=ROOT / 'build') as temporary:
            ...
            self.assertEqual(data, bytes.fromhex('b8 03 00 cd 10 c3'))
