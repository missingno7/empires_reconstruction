...
class RawDataWave139Tests(unittest.TestCase):
    def test_boundary_zero_runs_are_exact(self):
        manifest = read_json(ROOT / 'layout/manifest.json')
        recipe = read_json(ROOT / 'recipes/c/matching-wave139.json')
        evidence = read_json(ROOT / 'docs/matching-wave139-evidence.json')
        self.assertEqual(len(recipe['owners']), 2)
        self.assertEqual(sum(o['end'] - o['start'] for o in recipe['owners']), 11)
