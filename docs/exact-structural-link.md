...
topology in source. Ordered symbolic contributions now own 19,914 BSS bytes,
including the row-pointer, record, note, animation, and eleven-voice pointer
tables, the 672-byte resource-25 workspace, a 64-byte indexed control-state block, the 2,770-byte `s8c12` resource buffer, and the 684-byte `g96e4`/`g96ee` resource-render state block; anchored `GAMEBSS` fragments retain the 17,336-byte remainder. The arithmetic adapter changes only the order of explicit OMF fixup
