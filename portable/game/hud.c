...
void hud_icons_load()
{
    resource_load_record_alloc(0x3f, (uint8_t **)&gc0ee);
}
...
void hud_panel_open(void)
{
    ...
    hud_prompt_kind = 1;
    gc0fc = 1;
    gfx_blit_bitmap(6, 0xa2, (const uint8_t *)(gc0ee + *(dos_int *) gc0ee + 2));
    hud_draw_meter();
    hud_tab_draw();
    energy_draw();
    f7417();
...
/* PORT: the historical local re-declaration `extern char *gc0ee, *gc0f6,
   *gc0f2;` (near, distinct from the far view used by F_6FCA/F_6FDA) exists
   purely for TC's ABI (the comment: "would change the pushes the call below
   compiles to").  gc0ee/gc0f6/gc0f2 are generated as plain real pointers
   (portable/generated/game_state.h: dos_char *gc0ee, *gc0f6, *gc0f2) -- far
   pointers already flattened, so there is only one portable view; the
   umbrella game_state.h declaration is used throughout instead of a local
   redeclaration. */
...
void ui_overlay_hide(void)
{
    ...
    switch (hud_prompt_kind) {
    case 1:
        gfx_blit_bitmap(14, 0xb8, (const uint8_t *)(gc0ee + ((dos_uint *)gc0ee)[2] + 2));
...
void hud_draw_meter()
{
    ...
    for (i = 0; i < n; i++) {
        gfx_blit_bitmap(x, 0xb0, (const uint8_t *)(gc0ee + ((dos_uint *)gc0ee)[1] + 2));
        x += 18;
    }
    if (gc0fa)
...
void hud_tab_draw()
{
    gfx_blit_bitmap(0x98, 0xa6, (const uint8_t *)(gc0ee + ((dos_uint *)(gc0ee + 6))[gb7e] + 2));
    if (gb7e == 2)
        gfx_blit_bitmap(0xa6, 0xae, (const uint8_t *)(gc0ee + ((dos_uint *)(gc0ee + 12))[gb80] + 2));
    if (gc0fa)
...
void energy_set(dos_int value) { energy_meter = value; }
...
struct R7417 { dos_char pad[22]; dos_uint offsets[1]; };

void f7417(void) { gfx_blit_bitmap(244,175,(const uint8_t *)((dos_char *)gc0ee + ((struct R7417 *)gc0ee)->offsets[tick_div8()] + 2)); }


/* ---- F_7443 (original code at 0x7443) ---- */
struct R7443 { dos_char pad[32]; dos_uint offsets[1]; };

void hud_panel_node_marker_draw(void) { dos_int i; i=campaign_node_index(); gfx_blit_bitmap(244+i*16,186,(const uint8_t *)((dos_char *)gc0ee + ((struct R7443 *)gc0ee)->offsets[i] + 2)); }
