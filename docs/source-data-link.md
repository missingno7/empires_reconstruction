...
is now byte-identical. TEXT placement remains unchanged, and only 32 TEXT bytes
differ. TLINK emits 72 of the original 106 relocation sites, with no extra sites.
...
37,252-byte BSS scaffold is unchanged; BSSEND is still two bytes high. Local
...
remaining code bindings, MZ fields and relocation order are still open.
