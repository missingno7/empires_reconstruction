"""Fresh complete-extent proof for the recovered F_6CF0 C routine."""
...
class MatchingCWave56Tests(unittest.TestCase):
    def test_recovered_complete_extent_and_fixups(self):
        manifest = read_json(ROOT / 'layout/manifest.json')
        recipe = read_json(ROOT / 'recipes/c/matching-wave56.json')
        owner = next(r for r in manifest['regions'] if r['id'] == 'F_6CF0')
        self.assertEqual(next(r for r in recipe['owners'] if r['id'] == 'F_6CF0'), owner)
        ...
        with tempfile.TemporaryDirectory(dir=ROOT / 'build') as temporary:
            ...
            self.assertEqual(data, bytes.fromhex('a1 e8 c0 eb 00 c3'))
