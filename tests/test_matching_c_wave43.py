"""Fresh complete-extent proof for the F_2986 far-pointer helper."""
...
class MatchingCWave43Tests(unittest.TestCase):
    def test_fresh_far_helper_extent_and_fixups(self):
        manifest = read_json(ROOT / 'layout/manifest.json')
        owner = next(r for r in manifest['regions'] if r['id'] == 'F_2986')
        ...
        with tempfile.TemporaryDirectory(dir=ROOT / 'build') as temporary:
            ...
            self.assertEqual(len(data), 167)
            self.assertEqual(len(proof['fixups']), 7)
