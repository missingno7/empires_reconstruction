"""Fresh complete-extent proof for the recovered F_6CA6 resource-table C routine."""
...
class MatchingCWave68Tests(unittest.TestCase):
    def test_recovered_complete_extent_and_fixups(self):
        manifest = read_json(ROOT / 'layout/manifest.json')
        recipe = read_json(ROOT / 'recipes/c/matching-wave68.json')
        owner = next(r for r in manifest['regions'] if r['id'] == 'F_6CA6')
        self.assertEqual(next(r for r in recipe['owners'] if r['id'] == 'F_6CA6'), owner)
        ...
        with tempfile.TemporaryDirectory(dir=ROOT / 'build') as temporary:
            ...
            self.assertEqual(len(proof['fixups']), 8)
            self.assertEqual(proof['load_relocations'], [])
            self.assertEqual(data[:8], bytes.fromhex('55 8b ec 56 8b 5e 04 89'))
            self.assertEqual(data[-6:], bytes.fromhex('36 de c0 5e 5d c3'))
